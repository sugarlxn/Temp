    ...
    out ingress_intrinsic_metadata_t  ig_intr_md)
{
    ...
    /*有限状态机*/
     state start {
        ...
        pkt.advance(PORT_METADATA_SIZE);//跳过端口元数据 指针移动
    ...
    state parse_vlan_tag {
        ...
        transition select(hdr.vlan_tag.ether_type) {
            ETHERTYPE_IPV4:  parse_ipv4;
            default: accept; //NOTE 这个accept状态时最后的状态
        }
    }
    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    //动作 drop
    action drop() {
        ig_dprsr_md.drop_ctl = 1;//丢包，控制为设置为1
    }
    //动作 组播
    action multicast(MulticastGroupId_t mcast_grp) {
        ig_tm_md.mcast_grp_a = mcast_grp;//指定组播组 
    }
    
    table ipv4_host {
        key = { hdr.ipv4.dst_addr : exact; } //NOTE exact 精确匹配
        actions = {
            send; drop; multicast; //NOTE在这里添加action multicast
#ifdef ONE_STAGE
...
#endif    
    table ipv4_lpm {
        key     = { hdr.ipv4.dst_addr : lpm; } //NOTE最长前缀匹配
        actions = { send; drop; multicast; }
        
        default_action = send(64); //默认动作 copy to cpu 默认发64端口逻辑端口
        size           = IPV4_LPM_SIZE;
    }
...
{
    //动作 修改报文分为两种情况 有vlan 和 无vlan
    action modify_packet_vlan(bit<12> vlan_id, bit<48> dstmac, bit<32> dstip)
    {
