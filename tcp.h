...
//send window
// int tcp_window_handle(struct tcp_stream * stream, struct tcp_fragment * fragment);
...
struct tcp_window 
{
	...
	struct tcp_packet_node* middle;//表示窗口的中间，#3的第一个tcp_packet_node，窗口内未发送的tcp报文
	struct tcp_packet_node* tail;//表示窗口的尾部，#4的第一个tcp_packet_node，窗口内最后一个tcp报文
