...
{           
	...
	stLocalAddr.sin_addr.s_addr = inet_addr("172.31.196.226"); 
...
int tcp_client_entry(__attribute__((unused))  void *arg)
{
	...
	while (1)
	{
		...
		// scanf("%s", sendbuf);
		// int data_len = strlen(sendbuf);
		// printf("send data: %s;length:%d\n",sendbuf,data_len);
...
static void timer1_cb(__rte_unused struct rte_timer* timer, __rte_unused void* arg){
	...
	rte_timer_reset(timer, hz/3, SINGLE, lcore_id, timer1_cb, NULL);
}
...
int main(int argc, char *argv[]) 
{
	...
	*/
	...
	rte_get_next_lcore(uiCoreId, 1, 0);
	rte_timer_reset(&timer0, hz, PERIODICAL, uiCoreId, timer0_cb, pstMbufPoolPub);
	rte_get_next_lcore(uiCoreId, 1, 0);
	rte_timer_reset(&timer1, hz/3, SINGLE, uiCoreId, timer1_cb, pstMbufPoolPub);
