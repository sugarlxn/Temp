...
static int tcp_handle_established(struct tcp_stream *pstStream, struct rte_tcp_hdr *pstTcphdr, int iTcplen) 
{
	...
	//FIXME 
	if(pstTcphdr->tcp_flags & RTE_TCP_ACK_FLAG)  //异常
	{
...
//tcp 发送窗口管理函数，返回0表示发送成功，-1表示发送失败
static int tcp_window_handle(struct tcp_stream * stream, struct tcp_fragment * fragment){
	
	if (stream == NULL || fragment == NULL){ 
		return -1;
	}
	...
	//单链表插入 insert 尾插法
	//此时单链表为空
	if(stream->window->head == NULL && stream->window->middle == NULL && stream->window->tail == NULL){
		stream->window->head = packet_node;
		stream->window->middle = packet_node;
		stream->window->tail = packet_node;
	}
	else{ //此时单链表不为空
		stream->window->tail->next = packet_node;
		stream->window->tail = packet_node;
	}

	//更新发送窗口大小
	stream->window->window_used += fragment->length;
